...
//Llamo el SP para agregar un alumno que tengo en la BD
    ...
    @$sql = "CALL agregar_turno ('".$descripcion."',".$activo.");";
    $res = mysql_query($sql);
    $rs = mysql_fetch_assoc($res);
    
    if ($rs["estado"] == "nuevo") {
        
        $cargado = "Se agrego un turno nuevo: ".$rs["descripcion"];
        
    } elseif ($rs["estado"] == "cargado") {
        $cargado = "Ya se encuentra registrado un turno con esa descripcion: ".$rs["descripcion"];
    }
