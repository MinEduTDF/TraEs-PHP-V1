...
//Llamo el SP para agregar un alumno que tengo en la BD
    ...
    @$sql = "CALL agregar_secundario ('".$descripcion."',".$activo.");";
    $res = mysql_query($sql);
    $rs = mysql_fetch_assoc($res);
    
    if ($rs["estado"] == "nuevo") {
        
        $cargado = "Se agrego un colegio secundario nuevo: ".$rs["descripcion"];
        
    } elseif ($rs["estado"] == "cargado") {
        $cargado = "Ya se encuentra registrada un colegio secundario con esa descripci&oacute;n: ".$rs["descripcion"];
    }
