...
//Llamo el SP para agregar un alumno que tengo en la BD
    ...
    @$sql = "CALL agregar_primaria ('".$descripcion."',".$activo.");";
    $res = mysql_query($sql);
    $rs = mysql_fetch_assoc($res);
    
    if ($rs["estado"] == "nuevo") {
        
        $cargado = "Se agrego una escuela primaria nueva: ".$rs["descripcion"];
        
    } elseif ($rs["estado"] == "cargado") {
        $cargado = "Ya se encuentra registrada una escuela primaria con esa descripci&oacute;n: ".$rs["descripcion"];
    }
